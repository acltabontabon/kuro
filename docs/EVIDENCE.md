...
- Trust posture this layer serves: [TRUST_AND_TRANSPARENCY.md](./TRUST_AND_TRANSPARENCY.md). Principle 4 — "KURO always shows evidence: no Evidence, no Signal" — is enforced here via `Signal.evidenceIds.min(1)` and the referential-integrity checks, and underpins the evidence-transparency requirements in §6 of that document.
